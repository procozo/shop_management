...
class Drawflow {
  ...
  load() {
    ...
    for (var key in this.drawflow.drawflow[this.module].data) {
      // console.log("updating", this.drawflow.drawflow[this.module].data[key]);
  ...
  click(e) {
    ...
    //     console.log('clicked the output', e.target.classList[1])
        ...
        if (output_id !== input_id && input_class !== false) {
          if (
            ...
            this.drawflow.drawflow[this.module].data[id_output].outputs[
              output_class
            ].connections.push({ node: id_input, output: input_class, relation: '' });
            this.drawflow.drawflow[this.module].data[id_input].inputs[
              input_class
            ].connections.push({ node: id_output, input: output_class, relation: '' });
  ...
  drawConnection(ele) {
    ...
    // connection.appendChild("<text>dasd<text>");
    ...
    if (nodeOneModule === nodeTwoModule) {
      ...
      if (exist === false) {
        ...
        this.drawflow.drawflow[nodeOneModule].data[id_output].outputs[
          output_class
        ].connections.push({ node: id_input.toString(), output: input_class, relation: '' });
        this.drawflow.drawflow[nodeOneModule].data[id_input].inputs[
          input_class
        ].connections.push({ node: id_output.toString(), input: output_class, relation: '' });
  ...
  updateConnectionNodes(id) {
    // Aquí nos quedamos;
    console.log("updating", id, id[id.length - 1]);
    // var output_class = this.ele_selected.classList[1];
    ...
    if (elemsOut[elemsOut.length - 1]) {
      console.log(elemsOut[elemsOut.length - 1].classList[1][elemsOut[elemsOut.length - 1].classList[1].length - 1])
      let tempBonusID = elemsOut[elemsOut.length - 1].id
      console.log(tempBonusID)
      let tempID = elemsOut[elemsOut.length - 1].classList[1][elemsOut[elemsOut.length - 1].classList[1].length - 1]
      let targetnodeID = elemsOut[elemsOut.length - 1].classList[2][elemsOut[elemsOut.length - 1].classList[2].length - 1]
      console.log(this.drawflow.drawflow[this.module].data[tempID].inputs['input_1'].connections, targetnodeID);
      this.drawflow.drawflow[this.module].data[tempID].inputs['input_1'].connections.find((el) => el.node === targetnodeID).relation = tempBonusID
      // console.lolg(this.dra)
    }
    Object.keys(elemsOut).map(function (item, index) {
      if (elemsOut[item].querySelector(".point") === null) {
        ...
        // console.log(elemsOut[item].classList.value.split(" ").join("."));
        // console.log(
        //   ".connection.node_in_node-2.node_out_node-1.output_1.input_1"
        // );
        let label1 = document.querySelector(
          "." + elemsOut[item].classList.value.split(" ").join(".")
        );
        // let label1 = document.querySelector(
        //   ".connection.node_in_node-2.node_out_node-1.output_1.input_1"
        // );
        console.log(elemsOut[item]);
        let idValue = elemsOut[item].id
        if (idValue) {
          addLabelText(label1, idValue);
        }
        function addLabelText(bgPath, labelText) {
          ...
          textElemPath.setAttribute("text-anchor", "middle");
          textElemPath.setAttribute("startOffset", "50%");
          textElemPath.setAttribute("x", SVGRect.x);
          textElemPath.setAttribute("y", SVGRect.y);
          textElemPath.setAttribute("width", SVGRect.width);
          textElemPath.setAttribute("height", SVGRect.height);
          textElemPath.setAttribute("fill", "red");
          // textElemPath.setAttribute("filter", "url(#bg-text)");
          textElemPath.classList.add("label-text");
          textElemPath.textContent = labelText;
          textElem.appendChild(textElemPath);
          bgPath.appendChild(textElem);
  ...
  addNodeImport(dataNode, precanvas) {
    if (precanvas === null) {
      this.drawflow.drawflow["Home"].data[this.nodeId++] = dataNode;
    }
    // precanvas = precanvas ? precanvas : this.precanvas;
    ...
    Object.keys(dataNode.inputs).map(function (input_item, index, name, n) {
      const input = document.createElement("li");
      console.log(dataNode)
      if ((precanvas === null || dataNode.typeData === 'group') && index > 0) {
        input.classList.add("input_custom");
      } else {
        input.classList.add("input");
      }
      ...
      inputs.appendChild(input);
      input.innerHTML = dataNode.inputs[input_item].relation ? "<span>" + dataNode.inputs[input_item].name + "<b>" + dataNode.inputs[input_item].relation + "</b>" + "</span>" : "<span>" + dataNode.inputs[input_item].name + "</span>";
      // console.log(dataNode.inputs[input_item].name)
      Object.keys(dataNode.inputs[input_item].connections).map(function (
        output_item,
        index
      ) {
        console.log(dataNode.inputs[input_item].connections[output_item].relation);
        console.log(name);
        var connection = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "svg"
        ...
        if (dataNode.inputs[input_item].connections[output_item].relation !== null) {
          connection.id = dataNode.inputs[input_item].connections[output_item].relation
        }
        // connection.classList.add(dataNode.inputs[input_item].connections[output_item].relation)
    ...
    for (var x = 0; x < Object.keys(dataNode.outputs).length; x++) {
      const output = document.createElement("div");
      console.log(dataNode.outputs["output_" + (x + 1)].name);
      // output.classList.add("output");
      if ((precanvas === null || dataNode.typeData === 'group') && x > 0) {
        output.classList.add("output_custom");
      } else {
        output.classList.add("output");
      }
      output.classList.add("output_" + (x + 1));
      output.innerHTML =
        "<span>" + dataNode.outputs["output_" + (x + 1)].name + "</span>";
      outputs.appendChild(output);
    }
  ...
  removeNodeId(id) {
    ...
    console.log(this.drawflow.drawflow, moduleName);
    moduleName = moduleName ? moduleName : "Home";
    console.log(this.drawflow.drawflow[moduleName].data);
  ...
  on(event, callback) {
    ...
    console.log(this.events[event].listeners);
  ...
  /*Custom*/
  _take(event, callback) {
    console.log(event, callback);
    // this.dispatch('nodeMain', { node: ';' });
    // if (typeof callback !== 'function') {
